...
//!
//! Turnkey-крейта под UART нет (проверено — не нашлось ни одного, кто не
//! требовал бы пробника). Раз `defmt-bbq` отпадает (см. выше), паттерн
//! приходится держать своим — благо это буквально то, чем был бы
//! `defmt-bbq`, только против актуального `defmt@1.1.0`. Grant/commit API
