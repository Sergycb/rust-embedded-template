...
//!
//! Если у чипа хватило RAM, генерация отрезала от её конца килобайт и поделила
//! на два региона: `PANIC` — под дамп `panic-persist` (им занят
//! `#[panic_handler]` в `main.rs`, трогать не надо) и `PERSIST` — свободный,
//! под что угодно своё. Для второго в `memory.x` заведена секция `.persist`,
//! `NOLOAD`: образ её не инициализирует, а `cortex-m-rt` не обнуляет при
//! старте, поэтому значение переживает программный сброс — но не пропадание
//! питания, это по-прежнему RAM.
//!
//! ```ignore
...
//! ```
//!
//! Регионы разные не для порядка: `panic-persist` пишет по голым адресам
//! `_panic_dump_start.._panic_dump_end`, ничего не зная про секции, и в общем
//! регионе он затирал бы такие статики — молча, потому что линкеру наложение
//! символов на секцию не видно.
//!
//! Данные в `.persist` не инициализируются ничем, включая первый в жизни
//! платы старт: там будет то, что оказалось в RAM при подаче питания. Если
//! отличать «первый запуск» от «после сброса» важно, кладите рядом со
//! значением собственную магию и проверяйте её — ровно так же, как это делает
//! сам `panic-persist`.
