...
`Board::init()` — единственное место, где начинается владение железом. Оттуда приходят
не только периферия чипа, но и периферия ядра Cortex-M: `board.core` — это целиком
