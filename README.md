...
Сначала четыре вопроса: CI-провайдер (`github` / `gitlab` / `none`), нужен ли OTA
(bootloader с разделами `ACTIVE`/`DFU` — или один образ на весь flash), нужен ли раздел
под настройки во flash и надо ли проверять подпись OTA-образа. Потом — каскадный выбор
конкретного STM32. Порядок именно такой, поэтому вопросы и не ссылаются на ваш чип:
