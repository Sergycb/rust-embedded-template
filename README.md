...
`target` можно и переопределить по отдельности своим `--define` — например если
дефолтный `chip` указывает не на самый специфичный вариант probe-rs для вашего чипа.
