...
чипа. Список вариантов на каждом шаге — реальные чипы, которые одновременно поддерживают и
`embassy-stm32`, и установленный локально `probe-rs`. По результату автоматически
подставляются:
...
`--define chip_feature=stm32f407ve` — остальные три поля выведутся из него автоматически.

Дополнительно шаблон спросит:
...
хотите отвести под bootloader и DFU-раздел; `write_size` из списка выше — туда же.
