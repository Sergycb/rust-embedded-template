...
- `MEMORY {}` в обоих `cross/*/memory.x` — адреса и размеры `FLASH`/`BOOTLOADER_STATE`/
  `ACTIVE`/`DFU`/`RAM`/`PERSIST`, посчитанные по реальным границам секторов вашего чипа
  (источник — `stm32-metapac`, та же зависимость, что уже тянет `embassy-stm32`).
...
`memory.x`/`write_size` считаются не для всех ~1438 чипов — для слишком маленьких
flash/RAM (нет места на нормальный bootloader+ACTIVE+DFU) шаблон честно оставляет
`MEMORY {}` плейсхолдером на ручное заполнение, ничего не подставляя наугад.

**Двухъядерные чипы** (STM32H745/747/755/757, STM32WL54/55 — выбираются в каскаде явно,
отдельным вариантом на каждое ядро) — плейсхолдеры каскад подставляет верно, но
сгенерированный `cross` для них пока не собирается: `embassy_stm32::init()` требует другого
API для asymmetric multicore, шаблон его не реализует (осознанное ограничение, не баг).
...
(параметр буфера подкачки `BootLoader::prepare::<_,_,_,N>` — не путать с минимальной
порцией записи flash, у STM32 она всегда маленькая, 4-8 байт) и оба `memory.x`
вычисляются автоматически по выбранному чипу — см. «Генерация проекта» выше.
