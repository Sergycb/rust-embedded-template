...
impl Ota {
    ...
    /// Занят ли раздел `DFU` тем, что терять нельзя.
    ///
    /// [`State::Swap`] — обновление помечено к обмену, и в `DFU` лежит образ,
    /// который bootloader перенесёт в `ACTIVE` на ближайшем сбросе.
    /// [`State::Revert`] — обмен уже случился, но новый образ себя не
    /// подтвердил, и в `DFU` теперь предыдущая, заведомо рабочая прошивка:
    /// именно её bootloader вернёт, если подтверждения так и не будет.
    ///
    /// `Revert` здесь не для симметрии. README советует переносить
    /// `mark_booted()` туда, где устройство доказало работоспособность, — то
    /// есть в реальном проекте это окно живёт долго. Начатая в нём передача
    /// стирала образ отката, и провалившаяся самопроверка возвращала
    /// устройство в наполовину стёртый раздел, то есть в кирпич.
    ///
    /// `embassy-boot` в `verify_booted` `Revert` разрешает, и это его право:
    /// он отвечает на «можно ли писать вообще», а не на «не жалко ли того,
    /// что лежит в разделе». Здесь правило строже намеренно.
    fn dfu_is_busy(&mut self) -> Result<bool, Error> {
        Ok(matches!(self.state()?, State::Swap | State::Revert))
    }
...
{%- if signed == "true" %}
    ...
    fn refuse_rollback(&mut self, length: u32) -> Result<(), UpdateError> {
        // В состояниях `Swap` и `Revert` в `DFU` лежит не присланный образ, а
        // предыдущая прошивка (см. `dfu_is_busy`). Версия у неё заведомо
        // старее, так что проверка ниже честно ответила бы `Rollback` — и
        // соврала бы о причине: на деле раздел трогать нельзя вовсе. Разные
        // отказы должны быть различимы, поэтому этот случай отделяется здесь.
        if self.dfu_is_busy()? {
            return Err(Error::BadState.into());
        }
...
impl ports::FirmwareUpdate for Ota {
    ...
    fn write(&mut self, offset: u32, data: &[u8]) -> Result<(), Self::Error> {
        ...
        // состояние (см. `dfu_is_busy`). Прямая запись в раздел её не делает,
        // а без неё приложение затёрло бы образ, в который bootloader
        // собирается откатиться на ближайшем сбросе.
        //
