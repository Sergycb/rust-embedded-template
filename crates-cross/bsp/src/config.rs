...
//! стоит 256 KiB.
//!
//! Зачем это в шаблоне. `miniconf` в `domain` описывает дерево настроек
//! («адресуемое дерево, доступ к листьям по JSON-пути»), но класть его было
//! некуда: в `memory.x` не было ни одного сектора под данные, и любая
//! калибровка жила до первого сброса. Здесь — только хранилище; чем
