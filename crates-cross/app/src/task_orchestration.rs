...
//! `ports::FirmwareUpdate` (не забудьте импорт): `board.ota.prepare()` один раз
//! в начале передачи — он стирает раздел, — и `board.ota.write(offset, chunk)`
//! в цикле приёма. Без первого запись ляжет в нестёртую память, то есть
//! мусором и молча. Сам приём естественно ложится на узел графа выше —
