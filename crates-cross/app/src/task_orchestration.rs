...
{%- if graph != "true" %}
//! Писать её руками не нужно; собрать объект и кормить его — нужно, потому
//! что графа в этом проекте нет, а мультиплексор кормлений живёт именно в
//! нём. Имя блока при этом уже подставлено генерацией из метаданных чипа
//! (`IWDG`, на H7 `IWDG1`, на двухъядерных H7 у ядра CM4 `IWDG2`).
{%- else %}
...
//!
//! Понадобится автомат состояний (например, крейты `fsm`/`fsm-async` из
//! [rust-lib](https://github.com/Sergycb/rust-lib)) — его место в `domain`, а
...
//! Стыкуется он с готовой частью через `domain::download::Download`: тот
//! сверит длину с вместимостью ДО стирания, подготовит раздел, соберёт куски
//! до слова флеша и не даст образу вылезти за обещанную длину. Писать этот
//! цикл руками не стоит — половина здешних граблей не видна, пока в них не
//! попадёшь (перечислены в doc-комментарии того модуля). Сам
...
//! ```
//!
//! Четыре вещи, на которых спотыкаются:
//!
...
//! * **Пробник в STOP отваливается.** RTT — это чтение памяти отладчиком, а в
//!   STOP ядро остановлено и отладочный порт по умолчанию обесточен: лог
//!   замолкает, `probe-rs attach` рвётся. Лечится `config.enable_debug_during_sleep`
//!   у `embassy_stm32::Config` (ток при этом вырастет — на время отладки).
//!   На это натыкаются раньше всего остального.
