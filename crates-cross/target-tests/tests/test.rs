...
mod tests {
    ...
    /// Замер стека даёт правдоподобное число — то есть узор лёг туда, куда
    /// растёт стек, а не в чужую память.
    ///
    /// Проверяются обе границы, и каждая ловит свою поломку. Ноль означает, что
    /// узора не нашлось у самой нижней границы: либо заливки не было, либо
    /// стек оказался не там, где его ищут (например, `flip-link` перестал быть
    /// линкером). Целый стек означает обратное — узор нигде не тронут, то есть
    /// залили не тот участок: сам этот тест уже потратил на вызовы хоть
    /// сколько-то.
    ///
    /// Границы берутся у `bsp::stack::span()`, а не числом: у STM32 бывает и
    /// два килобайта RAM, и три мегабайта, так что любая константа здесь
    /// оказалась бы неверна на одном из концов линейки.
    ///
    /// Заодно печатается сам расход: на плате это единственный способ узнать,
    /// сколько стека осталось про запас.
    #[test]
    fn stack_usage_is_measurable(_board: Board) {
        let free = bsp::stack::never_used();
        let span = bsp::stack::span();
        defmt::info!("стека не тронуто: {} из {} байт", free, span);

        assert!(
            free > 0,
            "у нижней границы стека нет узора: заливка не сработала или стек не там, \
             где его ищут (проверьте, что линкер — flip-link)"
        );
        assert!(
            free < span,
            "нетронутыми числятся все {} байт стека — узор искали не там",
            span
        );
    }
    ...
    /// Дамп аппаратного отказа читается тем же читателем, что и дамп паники.
    ///
    /// Единственная проверка того, что формат записи в `bsp::fault` сходится
    /// с форматом `panic-persist`. Публичного API записи тот не отдаёт —
    /// принимает только `&PanicInfo`, которого для аппаратного отказа не
    /// бывает, — поэтому раскладка воспроизведена вручную, а сойтись стороны
    /// обязаны: читает-то запись `Board::last_panic`, то есть сам
    /// `panic-persist`. Разъедься форматы при обновлении крейта, `HardFault`
    /// молча перестал бы оставлять след, и выяснилось бы это на первом
    /// падении в поле.
    ///
    /// Настоящий отказ для проверки не нужен и не годится: вызвать его прямо
    /// в тесте значило бы увести весь прогон — обработчик под debug паркует
    /// ядро, и `probe-rs run` не дождался бы ни этого теста, ни следующих.
    #[test]
    fn a_hard_fault_dump_reads_back_as_a_panic_message(_board: Board) {
        // Осушить регион: в нём мог остаться дамп прошлого запуска, а
        // `record` намеренно не затирает уже стоящую запись.
        _ = Board::last_panic();

        bsp::fault::record(0x0800_1234, 0xFFFF_FFF9, 0x6100_0000);

        let dump = Board::last_panic().expect(
            "после record в PANIC должна лежать запись, которую видит panic-persist: не видит — \
             разошлись формат записи (bsp::fault) и формат чтения (panic-persist)",
        );
        assert!(
            dump.starts_with("HardFault"),
            "запись прочиталась, но это не наш дамп: {}",
            dump,
        );
        assert!(
            dump.contains("08001234"),
            "в дампе нет адреса отказа: {}",
            dump
        );
    }

    /// Второй отказ не затирает причину первого.
    ///
    /// Не формальность: под debug-профилем паникёр заканчивается `udf()`, то
    /// есть аппаратным отказом, — и без этого правила настоящая причина
    /// паники подменялась бы бесполезным «HardFault по адресу udf» на каждом
    /// падении.
    #[test]
    fn a_second_fault_keeps_the_first_reason(_board: Board) {
        _ = Board::last_panic();

        bsp::fault::record(0x1111_1111, 0, 0);
        bsp::fault::record(0x2222_2222, 0, 0);

        let dump = Board::last_panic().expect("первая запись должна остаться на месте");
        assert!(
            dump.contains("11111111") && !dump.contains("22222222"),
            "вторая запись затёрла первую: {}",
            dump,
        );
    }
