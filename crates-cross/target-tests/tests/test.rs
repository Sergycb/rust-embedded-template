...
mod tests {
    ...
    /// линкером). Число размером со всю RAM означает обратное — узор цел
    /// целиком, то есть залили не тот участок.
    ///
    ...
    fn stack_usage_is_measurable(_board: Board) {
        ...
        defmt::info!("стека не тронуто: {} байт", free);
        ...
        // Верхняя граница — вся RAM чипа. Точного размера тест не знает, но
        // мегабайта нет ни у одного STM32 из тех, что раздаёт шаблон.
        assert!(
            free < 1024 * 1024,
            "нетронутым числится {} байт — столько стека не бывает, узор искали не там",
            free
        );
    }
