...
не выводятся при RAM < 4 KiB (`PERSIST_MIN_RAM`): отрезать от такого
килобайт разорительно. Килобайт этот делится пополам — `PERSIST` под данные
программы (секция `.persist`), `PANIC` под дамп `panic-persist`, который пишет по
голым адресам и в общем регионе затирал бы секцию молча. Ручной
