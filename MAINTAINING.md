...
зависит) под каждую из шести конфигураций, задевающих разные ветки генерации:
обычный одноядерный, он же с `ota=no`, чип с выбором банковой схемы, чип, куда OTA не
помещается, двухъядерный (`init_primary` вместо `init`) и чип с 2 KiB RAM на
...
сгенерированном проекте (`cargo update` для обоих воркспейсов). Ломающие изменения
...
конфигурации и только на ней: граф пакетов от чипа не зависит, а шесть обновлений
...
- сам `template-check` генерирует не из репозитория, а из его копии, куда не
  ...
  двух мегабайт, полный `--quick` по шести конфигурациям укладывается в двадцать
...
манифесту. Локальная сборка чинит это молча, а `cargo fetch --locked` — первый шаг в
обоих CI — падает с «cannot update the lock file ... because --locked was passed»
(проверено на обоих вариантах). Держать в шаблоне четыре lock-файла под все сочетания
ответов дороже, чем один раз пересобрать в проекте, поэтому post-хук в таких случаях
печатает предупреждение с командой. Пользовательский CI при этом остаётся строгим —
`--locked` там нужен ровно для того, чтобы ловить рассинхрон.
...
`chip-data-gen` (включая 12 тестов каскада по всем чипам и проверку штампа версий) и
...
Кстати о том, почему `--quick` в CI вообще работает: `cargo update` внутри post-хука
там падает — доступа к `rust-lib` нет, — но хук ловит ошибку и печатает
предупреждение вместо того, чтобы уронить генерацию. Ровно ради этого он и обёрнут в
`try` (проверено на копии шаблона с заведомо битым git-URL).
