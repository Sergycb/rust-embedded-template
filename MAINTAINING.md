...
`ota` — единственный плейсхолдер, который хук не просто читает, а перезаписывает.
Спрашивается он строкой `yes`/`no` (тип `bool` не годится: `variable::get` в rhai
...
окончательное: один образ на весь flash, `cross/boot` и `cross/bsp/src/ota.rs`
удаляются, даже если схема в чип помещается. «Да» означает лишь «не возражаю»:
