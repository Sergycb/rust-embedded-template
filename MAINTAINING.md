...
### Почему кеш проверки лежит вне репозитория
...
лежит в репозитории, — включая `target/` с гигабайтами артефактов — копируется при
каждой генерации из локального пути.

Поэтому `template-check` держит и рабочие каталоги, и общий кеш сборки в
`%TEMP%/rust-embedded-template-check/`. Когда кеш жил в `target/`, он за десяток
прогонов дорастал до девяти гигабайтов, и обычная `cargo generate --path` по этому же
репозиторию сначала долго «висела», а потом падала с «Недостаточно места на диске
(os error 112)».

Остаток `target/` (от `cargo xtask lint` и `test host`) копируется по-прежнему —
это пара гигабайтов и несколько секунд. Если генерируете из локального пути часто,
держите его в узде или берите шаблон через `--git`.
