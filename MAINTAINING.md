...
подставляемые Liquid-рендером cargo-generate. Из них в `[placeholders]` объявлены
только `ci` и `ota`; все пять остальных выставляет `variable::set` в `[hooks].pre`-скрипте
