...
//! крупных (вытесняет её), плюс подпись образа. Итого девять — точный список
//! в `default_cases()`. Свой набор чипов — позиционными аргументами.
...
fn check_one(
    ...
    // А обновляется первая конфигурация — та самая, для которой посчитан
    // crates-cross/Cargo.lock шаблона.
    if refresh {
...
/// проекта — в полном прогоне их там только что пересобрал `cargo update`
/// (см. `refresh` выше), в `--quick` сравниваются как есть.
///
/// Расхождение почти всегда значит одно: манифест правили, а lock не
/// перегенерировали. Само по себе это тихо: в репозитории шаблона `cross`
/// никто не собирает, а вот CI сгенерированного проекта зовёт `cargo fetch
...
///
/// Зовётся только для конфигурации по умолчанию — и это существенно для
/// `crates-cross/Cargo.lock`: при `ota=no` из графа уходят `embassy-boot*`,
/// при `config=yes` добавляются `sequential-storage` с
/// `embedded-storage-async`, при `signed=yes` — `salty`. Это не рассинхрон, а
/// ожидаемое свойство варианта, и предупреждение на них только приучало бы
/// пролистывать настоящее.
