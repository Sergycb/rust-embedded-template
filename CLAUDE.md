...
Список валидных чипов в `chip-select.rhai` (между маркерами `BEGIN/END GENERATED CHIP
LIST`) генерирует `chip-data-gen` — отдельный standalone-крейт в корне репозитория, НЕ
