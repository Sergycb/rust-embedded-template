...
2. **`default-features = false`**, если у крейта есть тяжёлые/std/alloc-зависимые
   дефолтные фичи — включать только реально нужные.
3. **`domain` не отключать `default-features` не глядя.** У `domain` `default =
   ["embassy"]` — это не просто удобство, а обязательное условие корректной
   кодогенерации `hsmc::statechart!` (см. предупреждение в `domain/Cargo.toml` у
   фичи `"embassy"`). Любая новая зависимость на `domain` с
   `default-features = false` должна явно включать `features = ["embassy"]`
   обратно, иначе `hsmc` молча сгенерирует урезанный sync-вариант без `run()`.
4. **Каждая новая библиотека в `domain` — с примером** в `domain/examples/*.rs`,
...
5. **Каждая новая библиотека в `cross`, для которой ещё нет компилируемого
...
6. **Если библиотека не подошла** (не собирается на ARM/требует alloc без
...
7. **Перед тем как исключить зависимость из-за `no_std`/`no-alloc` ограничения** —
...
- `cross/app` и `cross/boot`: `debug` (defmt+RTT+`panic-probe`) и `release`
  ...
  одноимённые `defmt`/`log` фичи дальше.
