...
**Известное ограничение** (осознанное, не забытая деталь, зафиксировано по решению
пользователя — не чинить без отдельного запроса): каскад для этих ~60 двухъядерных чипов
подставляет chip/chip_feature/cpu/target корректно, но сгенерированный `cross` для них не
собирается — `embassy_stm32::init()`, которым безусловно пользуются `cross/app`/
`cross/boot`, в самом `embassy-stm32` спрятан за `#[cfg(not(feature = "_dual-core"))]`
(`E0425: cannot find function init`). Asymmetric multicore требует другого API
инициализации и отдельного архитектурного решения, что `cross/app` вообще делает на
двух ядрах — не однострочный фикс. Проверено (`cargo generate` → `cargo xtask build`)
на `stm32h745zi-cm7`. Однопроцессорные чипы (в т.ч. остальные H7 — H743/H753 и т.п.) не
затронуты.
