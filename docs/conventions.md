...
3. **Каждая новая библиотека в `cross`, для которой ещё нет компилируемого
   ...
   в `docs/modules/app-task-orchestration.md` (оркестрация) или
   `docs/modules/bsp-buffers.md` (буферы/периферия), с обоснованием «почему
...
- **Тактирование шаблон не настраивает и настраивать не будет.**
  ...
  `Board::init` и `EXPECTED_SYS_HZ` в target-тесте (см. `docs/clocks.md`,
