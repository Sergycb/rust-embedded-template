...
/// Корень проекта — два уровня вверх от манифеста: сам крейт лежит в
/// `crates-host/chip-info`.
///
/// От `CARGO_MANIFEST_DIR`, а не от текущего каталога: команду зовут через
/// `cargo xtask pins` откуда угодно, и `current_dir()` указывал бы куда
/// попало.
...
/// Проверяет, что чип в `crates-cross/Cargo.toml` — всё ещё тот, под который собран
/// справочник.
///
/// Чип-фича попадает сюда при генерации, а `crates-cross/Cargo.toml` пользователь
...
fn warn_on_chip_drift() {
    let manifest = project_root()
        .as_deref()
        .map(|root| root.join("crates-cross").join("Cargo.toml"));
    ...
    if !text.contains(&format!("\"{CHIP_FEATURE}\"")) {
        eprintln!(
            "ВНИМАНИЕ: в {} нет чип-фичи \"{CHIP_FEATURE}\", под которую собран справочник.\n\
			 Похоже, чип проекта сменили — тогда поправьте его и в crates-host/chip-info/Cargo.toml,\n\
