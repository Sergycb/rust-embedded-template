...
///
/// `None` в шаблоне — по той же причине, по которой `None` стоит в
/// `EXPECTED_SYS_HZ` у target-теста тактирования: чип шаблон знает, а сколько
/// места этот конкретный проект хочет оставить про запас — нет. Пока здесь
/// `None`, работает прежнее поведение: не влезло в раздел — ошибка, подошло к
/// девяти десятым — предупреждение.
///
/// Заполните — и `cargo xtask build` начнёт падать, стоит образу перерасти
/// бюджет. Смысл в том, чтобы узнать об этом на сборке, а не при первом OTA:
/// в раздел `DFU` образ ещё влезет, а вот места под следующее обновление уже
/// не останется — и устройство в поле окажется без пути назад. Цифру берут не
/// с потолка, а из запаса на оставшиеся функции; 85 — разумная отправная
/// точка для проекта, который ещё растёт.
///
/// Проверка работает и локально, и в CI одинаково — именно поэтому она
/// константа в коде, а не переменная окружения CI-джоба: порог это свойство
/// проекта, а не свойство запуска.
...
/// Все проверки, которым не нужна плата, — в порядке из AGENTS.md, до первой
/// упавшей.
///
...
fn fault_field(dump: &str, name: &str) -> Option<u32> {
    ...
    let digits: String = dump[at..]
        ...
        .collect();
    // Ровно восемь цифр — столько печатает `bsp::fault` для любого значения
    // (`{:08x}` от `u32`). Меньше означает, что строка кончилась посреди поля:
    // регион `PANIC` оказался мал (раскладку заполняли руками — генерация даёт
    // не меньше 128 байт), и хвост дампа обрезан. Разобрать такой огрызок
    // значило бы напечатать уверенно неверный адрес отказа вместо честного
    // «поля нет».
...
/// Сколько шестнадцатеричных цифр в одном поле дампа: `u32` под `{:08x}`.
...
mod tests {
    ...
    /// Значение шаблона по умолчанию: раздел, занятый под завязку, сборку не
    /// роняет. Иначе первый же проект, которому места хватает впритык,
    ...
    /// огрызку поля.
    ///
    /// Регион бывает мал (раскладку заполняли руками), и тогда строка
    /// кончается на середине последнего значения. Показать «адрес отказа
    /// 0x2000» вместо честного молчания — худшее, что здесь можно сделать:
    /// человек пойдёт искать по нему.
    #[test]
    fn a_truncated_field_is_not_guessed() {
        let cut = "HardFault pc=08001234 lr=fffffff9 psr=61000000 cfsr=00008200 hfsr=40000000 \
                   mmfar=00000000 bfar=2000";
        let lines = super::decode_fault(cut).join("\n");

        // Биты разобрались — их поля целы.
        assert!(
            lines.contains("PRECISERR"),
            "потерян разбор целых полей: {lines}"
        );
        // А строки с адресом нет вовсе. Проверяется именно её отсутствие, а не
        // отсутствие цифр: `{:#010x}` дополняет значение нулями слева, так что
        // «0x2000» не встретилось бы в выводе и без этой защиты — такая
        // проверка была бы ложно-зелёной (проверено удалением защиты).
        assert!(
            !lines.contains("адрес доступа"),
            "показан адрес, собранный из огрызка: {lines}"
        );
    }
