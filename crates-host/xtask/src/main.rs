...
fn ensure_signing_enabled() -> Result<(), anyhow::Error> {
    anyhow::ensure!(
        ...
        "подпись образов в этом проекте не включена: она выбирается при генерации \
         ...
         это фича `ed25519-salty` у embassy-boot в crates-cross/bsp/Cargo.toml, ключ в \
         crates-cross/bsp/src/ota.rs и вызов verify_and_mark_updated вместо mark_updated",
    );
    Ok(())
}

/// Создаёт ключевую пару и печатает открытый ключ в виде, готовом к вставке в
/// `crates-cross/bsp/src/ota.rs`.
///
...
fn ota_key() -> Result<(), anyhow::Error> {
    ...
    println!();
    println!("вставьте в crates-cross/bsp/src/ota.rs:");
...
/// `crates-cross/target/...`: ELF содержит секции и символы, которых во flash нет.
