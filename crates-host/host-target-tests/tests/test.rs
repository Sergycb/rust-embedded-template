...
/// Магия, которую прошивка кладёт в начало `PERSIST` (см. `MAGIC` в
/// crates-cross/bsp/src/persist.rs — там же описана раскладка региона: магия по
/// смещению 0, счётчик по смещению 4). Совпадение с ней означает «прошивка
/// дошла до `main` и успела поработать».
