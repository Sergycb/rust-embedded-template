...
// `pub`, а не приватный с одним реэкспортом: обоснование выбора датчика и
// разбор формата живут в doc-комментарии модуля, и приватный `mod` спрятал бы
// их от `cargo doc` — то есть ровно от тех, кому они адресованы.
pub mod lm75;
