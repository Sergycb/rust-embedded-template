...
impl Download {
    ...
    /// единицами: у стёртого флеша это «ничего не менять», и на образ такой
    /// добор не влияет — устройство считает хеш по обещанной длине.
...
mod tests {
    ...
    fn pads_the_last_word_of_an_unaligned_image() {
        ...
        assert_eq!(&flash.memory[..image.len()], &image[..]);
        // Хвост добит единицами — стёртым состоянием, а не мусором.
