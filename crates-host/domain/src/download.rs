...
pub enum Error<E> {
    ...
    Empty,
    /// Отказ самого флеша. `{0:?}`, а не `{0}`: порт не обещает `E: Display`
    /// (`ports::FirmwareUpdate::Error` — ассоциированный тип без бáунда), а
    /// `Debug` у него уже есть — этот же `derive` требует его для всего `Error<E>`.
    #[error("отказ флеша: {0:?}")]
    Flash(E),
}
...
impl Download {
    ...
    /// нулевое (см. docs/flash.md, «Стёртый флеш»), и на таких чипах добор —
